# Current Training Goal

*Source of truth for active training program, A-race target, and high-level phase map. Update when phase transitions occur or race plans change.*

## Active Program
**Off-season + base building** (Apr 27, 2026 – Jan 3, 2027)

Bridge between first half marathon (Music City 2026) and next A-race. Originally planned as eight months of fitness expansion + weakness work; restructured May 25, 2026 around a 12-week patellar tendinopathy rehab block ([2026-05-25-patellar-rehab-12wk.md](../programs/2026-04-off-season/plans/2026-05-25-patellar-rehab-12wk.md)) after a rising left-knee signal in Week 2 of the base phase.

Tracking stays at off-season cadence throughout — no daily metrics intake, even during the rehab block. The one structured weekly metric is the Saturday-morning decline squat test.

## Next A-Race
**Music City Half Marathon 2027** — April 24, 2027 (Nashville, TN)
- **Goal**: PR over 2026's 2:12:33 finish; finish strong and healthy
- **Specific build**: 16 weeks beginning Jan 4, 2027 (program folder: `/programs/2027-01-music-city-half/`, to be created)

## Tentative Tune-Up
**Nashville Half Marathon** — Oct 24, 2026 (Nashville, TN, runnash.com)
- Not committed yet — decide by late summer based on speed block results
- Function: race-fit fitness check, not a goal race

## Phase Map

*Restructured May 25, 2026 to accommodate the 12-week patellar rehab block. Speed block, fall tune-up, and transition base all push downstream. Nashville Half (Oct 24, 2026) no longer realistic as a tune-up — drop or reassess after rehab block. 2027 Music City build start date holds.*

| Phase | Window | Focus | Mileage Target |
|---|---|---|---|
| Recovery | Apr 27 – May 10 | Reverse taper. No running first 10-14 days. | 0 → 5 mpw |
| Off-season base *(ended early)* | May 11 – May 24 | Rebuild aerobic capacity. Cut short due to rising left-knee signal. | 14 → 16 mpw reached |
| **Patellar rehab block (HSR-led)** | **May 25 – Aug 16** | **3x/week HSR + symptom-gated Z2 running. No hills, no intensity.** | 10 → 24 mpw |
| Post-rehab base + intensity ramp | Aug 17 – Oct 11 | If rehab clean: rebuild base + reintroduce strides, light tempo | 24 → 28 mpw |
| Speed block (5K/10K) | Oct 12 – Dec 6 | VO2max + threshold work. 5K race in late fall as fitness check. | 25-28 mpw |
| Transition base | Dec 7 – Jan 3 | Easy mileage, holiday-friendly, prep for race build | 20-25 mpw |
| **2027 Music City build** | Jan 4 – Apr 24, 2027 | Specific 16-week race prep | Per phase plan |

## Near-term Benchmarks
- **5K time trial** (originally early June): **cancelled** — no hard efforts during rehab block. Reschedule into the post-rehab intensity ramp.
- **Weekly single-leg decline squat test** (Saturday mornings during rehab block): the primary objective progress marker
- **Fitness check at first race**: late-fall 5K (after speed block begins) — Nashville Half (Oct 2026) likely off the table given the shifted phase map; reassess at Aug 16 reassessment

---

*Detailed off-season phase plans will live in `/programs/2026-04-off-season/plans/` once structured.*
